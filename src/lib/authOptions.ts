...
async signIn({ user, account }) {
  if (account?.provider === 'google' && user.email) {
    try {
      ...
      if (dbUser.accountStatus !== 0) return false;
...
async jwt({ token, user, account, trigger, session }) {
  ...
  // Store provider tokens when present
  if (account) {
  ...
  if (t.email && (!t.uName || !t.role || !t.image || !t.id)) {
    try {
      ...
      console.log('jwt hydrated from DB:', { id: t.id, uName: t.uName, role: t.role, image: t.image, name: t.name });
    } catch (e) {
...
async session({ session, token }) {
  ...
  const customSession: CustomSession = {
    ...
    user: {
      ...
      role: t.role ?? null,
      image: t.image ?? null,
      isRegistered: (t.isRegistered ?? session.user?.isRegistered ?? true) as boolean,
    },
